...
public class MeetingController {
    ...
    @Operation(summary = "[회의록 작성 1단계] 제목, 날짜, 시간, 참여자, 서기, 파일, 회의 안건 저장",
            description =
                    "회의록 작성 첫 페이지에 대해서 적은 값들을 저장합니다.\n"
                    + "데이터와 파일을 같이 받아와야 해서 RequestPart를 사용했습니다.\n"
                    + "파일은 가공 없이 파일 그대로 보내주시면 백엔드 측에서 URL로 전환하여 후에 URL로 반환합니다.\n"
                    + "EX) https://actionpoint-bucket.s3.ap-northeast-2.amazonaws.com/meeting/reference/{UUID}-{OriginalFileName}.pdf\n"
                    + "Req : {프로젝트 ID, 회의 제목, 회의 날짜, 회의 시간, <List> 참석자 유저 ID, 작성자 유저 ID, <List> 회의안건 제목}, {<List> 파일}\n"
                    + "Res : <List> 회의안건 ID"
    )
    ...
    @Operation(summary = "[회의록 작성 2단계] 회의 안건에 따른 회의록 저장",
            description = "아젠다 ID 마다 TEXT 내용을 연결시킵니다.\n" +
                    "Req : <List> {회의안건 ID, 회의안건 내용}\n" +
                    "Res : 회의록 ID"
    )
    ...
    @Operation(summary = "[회의록 작성 3-1단계] 추가 논의 저장",
            description = "이 단계에서는 추가 논의와 액션포인트를 저장합니다.\n" +
                    "하지만 두 요청의 방식이 다르기 때문에 분리하도록 합니다. 한번에 두 방식의 요청을 보내주시기 바랍니다.\n" +
                    "Req : 회의 ID(URL Path), 추가 논의\n" +
                    "Res : ")
    ...
    @Operation(summary = "[회의록 작성 3-2단계] 액션 포인트 저장",
            description = "이 단계에서는 추가 논의와 액션포인트를 저장합니다.\n" +
                    "하지만 두 요청의 방식이 다르기 대문에 분리하도록 합니다. 한번에 두 방식의 요청을 보내주시기 바랍니다.\n" +
                    "Req : 회의 ID(URL Path), <List>{액션포인트 내용, 사용자 ID} -> 완료 상태는 요청 필요 X\n" +
                    "Res : ")
    ...
    @Operation(summary = "[회의록 조회]",
            description = "저장된 회의록에 대한 모든 정보를 넘깁니다. 수정 부분에서 사용됩니다.\n" +
                    "Req : 회의 ID (URL Path)\n" +
                    "Res : 회의 ID, 회의 제목, 회의 날짜, 회의 시간, 작성자 ID, 작성자 이름, <List> {참여자 ID, 참여자 이름}, <List> {참고자료 URL},\n" +
    ...
    @Operation(summary = "[회의록 수정]",
            description = "회의록은 추가 논의, 아젠다 내용, 액션포인트 내용만 수정 가능하며 액션포인트는 추가도 가능합니다.\n" +
                    "회의 ID 잊지 않고 넘겨주세요.\n" +
                    "PATCH로 액션포인트 수정과 생성을 한번에 처리할거라 두 액션포인트 부분을 따로 리스트화해서 보내주셔야 합니다.\n" +
    ...
    @Operation(summary = "[유저 리스트 조회]",
            description = "프로젝트에 해당하는 유저 리스트를 조회합니다. 언급을 통해 회의 참석자 및 작성자를 지정할 때 사용합니다.\n" +
                    "Req : 프로젝트 ID (URL Path)\n" +
